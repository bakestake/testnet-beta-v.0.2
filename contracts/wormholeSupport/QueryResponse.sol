...
abstract contract QueryResponse is Initializable, UUPSUpgradeable {
    ...
    function __Query_INIT_(address _wormhole) public initializer {
    ...
    function parseSolanaAccountQueryResponse(
        ParsedPerChainQueryResponse memory pcr
    ) public pure returns (SolanaAccountQueryResponse memory r) {
    ...
    function parseSolanaPdaQueryResponse(
        ParsedPerChainQueryResponse memory pcr
    ) public pure returns (SolanaPdaQueryResponse memory r) {
    ...
    function verifyQueryResponseSignatures(bytes memory response, IWormhole.Signature[] memory signatures) public view {
        ...
        (bool signaturesValid, string memory invalidReason) = wormhole.verifySignatures(
            responseHash,
            signatures,
            guardianSet
        );
